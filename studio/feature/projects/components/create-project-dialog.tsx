...
function getDefaultFormValues(): CreateProjectFormValues {
  return {
    title: '',
    goal: '',
...
export function CreateProjectDialog({
  ...
  const [form, setForm] = useState<CreateProjectFormValues>(getDefaultFormValues)
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription>
            填写项目信息。每周目标分数适用于所有周；默认 5 周，每周仅填内容，可增减周数。
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            <Label htmlFor="project-goal">每周目标分数</Label>
            <Input
              id="project-goal"
              type="number"
              min={0}
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              每周分数要求以项目「每周目标分数」为准；每行一周，仅填内容，可增减。
            </p>
