...
export function TodayExecutionCenter({ todos, plans, onPlanCheckIn, onPlanTargetChange, onTodosChanged, onExecutionChanged }: { todos: Todo[]; plans: LongTermPlanWithProgress[]; onPlanCheckIn: (planId: string) => Promise<void>; onPlanTargetChange: (planId: string, targetCount: number) => Promise<void>; onTodosChanged: () => void; onExecutionChanged: () => void }) {
  ...
  const action = (id: string, actionName: string) => executionAction({ id, action: actionName })
  const startTodo = (todoId: string) => executionAction({ todoId, action: 'startTodo' })
  ...
  const groupedLanes = useMemo(() => lanes.map((lane) => {
    const groups = new Map<string, { parent: Todo; parentInLane: boolean; tasks: Todo[] }>()
    const rootOf = (todo: Todo) => {
      let root = todo
      const visited = new Set([todo.id])
      while (root.parentId) {
        const parent = todoById.get(root.parentId)
    ...
    for (const todo of lane.items) {
      const parent = rootOf(todo)
      const group = groups.get(parent.id) ?? { parent, parentInLane: false, tasks: [] }
      if (todo.id === parent.id) group.parentInLane = true
      else group.tasks.push(todo)
    ...
    return {
      ...
      groups: [...groups.values()].map((group) => ({
        ...group,
        tasks: group.tasks.sort((a, b) => a.sortOrder - b.sortOrder || a.createdAt.localeCompare(b.createdAt)),
      })),
    }
  }), [lanes, todoById])
  const mapTaskCard = (todo: Todo, grouped = false) => <div key={todo.id} className={grouped ? 'border-l-2 border-violet-200 pl-2' : ''}><button type="button" onClick={() => selectTodo(todo)} className="block w-full rounded-md border bg-background p-2 text-left transition-colors hover:border-primary/40 hover:bg-muted/30"><p className="text-xs font-medium">{todo.title}</p><div className="mt-2 flex items-center justify-between gap-2"><span className="text-[9px] text-muted-foreground">{todo.status} · {todo.estimatedMinutes} 分钟</span><span role="button" tabIndex={0} onClick={(event) => { event.stopPropagation(); void promote(todo.id) }} className="text-[10px] font-medium text-primary">加入今天</span></div></button></div>
  const todaySubtasks = (item: WorkItem) => {
    ...
    return <div className="mt-3 border-t pt-2"><p className="mb-1.5 text-[9px] font-medium text-muted-foreground">子任务进度 · {children.filter((child) => child.status === 'done').length}/{children.length}</p><div className="space-y-1">{children.map((child) => { const workItem = data.items.find((candidate) => candidate.sourceType === 'todo' && candidate.sourceId === child.id && candidate.state !== 'skipped'); const state = workItem?.state ?? child.status; const done = child.status === 'done' || workItem?.state === 'done'; const running = workItem?.state === 'active'; return <div key={child.id} className="flex items-center gap-2 rounded-md bg-muted/50 px-2 py-1.5"><span className={`size-2 shrink-0 rounded-full ${done ? 'bg-emerald-500' : running ? 'bg-sky-500' : 'bg-muted-foreground/30'}`} /><button type="button" onClick={() => selectTodo(child, state)} className="min-w-0 flex-1 text-left"><span className={`block truncate text-[10px] font-medium ${done ? 'line-through opacity-60' : ''}`}>{child.title}</span><span className="block text-[9px] text-muted-foreground">{done ? '已完成' : running ? '进行中' : state === 'paused' ? '已暂停' : '待开始'} · {child.estimatedMinutes} 分钟</span></button><Button size="sm" variant="outline" disabled={done || running} onClick={() => void startTodo(child.id)} className="h-6 px-2 text-[9px]"><Play className="mr-1 size-2.5" />{workItem?.state === 'paused' ? '继续' : '进行'}</Button></div> })}</div></div>
  }
  return <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_18rem]">
    <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border bg-card">
      ...
      {view === 'today' ? <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {active ? <div className="mb-3 rounded-lg border border-sky-400 bg-sky-50/50 p-4"><p className="text-xs text-sky-700">正在执行</p><button onClick={() => selectWorkItem(active)} className="mt-1 text-left text-lg font-semibold">{active.title}</button><div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => void action(active.id, 'pause')} variant="outline"><Pause className="mr-1 size-4" />暂停</Button><Button onClick={() => void action(active.id, 'complete')}><Check className="mr-1 size-4" />完成</Button><Button onClick={() => void action(active.id, 'remove')} variant="ghost" className="text-muted-foreground">移出今天</Button></div>{todaySubtasks(active)}</div> : <div className="mb-3 rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">尚未开始任务</div>}
        <h3 className="mb-2 text-sm font-semibold">今日队列</h3><div className="space-y-2">{queue.map((item, index) => <div key={item.id} className="rounded-lg border p-3"><div className="flex items-center gap-3"><span className="w-5 text-center text-xs text-muted-foreground">{index + 1}</span><button onClick={() => selectWorkItem(item)} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-medium">{item.title}</p><p className="text-[10px] text-muted-foreground">{item.plannedMinutes} 分钟 · {item.priority}</p></button><Button size="sm" variant="outline" onClick={() => void action(item.id, 'start')}><Play className="mr-1 size-3" />开始</Button><Button size="sm" variant="ghost" onClick={() => void action(item.id, 'remove')} className="text-muted-foreground">移出</Button></div>{todaySubtasks(item)}</div>)}</div>
