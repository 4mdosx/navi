    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <DialogFooter>
          ...
          {!viewing && (
            <Button type="button" disabled={saving || !title.trim()} onClick={() => void save()}>
              {saving ? '保存中…' : '保存'}
            </Button>
          )}
        </DialogFooter>
...
function WeeklyControlPanel({
  ...
  onDragLeave: () => void
  onDrop: (event: React.DragEvent) => void
  actions: Pick<ReturnType<typeof useTodoStore.getState>, 'startTodo' | 'completeTodo' | 'addTodoTree' | 'updateTodo' | 'addSubtask'>
}) {
  const { startTodo, completeTodo, addTodoTree, updateTodo, addSubtask } = actions
  ...
  return (
    ...
    >
      ...
      <TodoDetailDialog item={detailTodo} open={detailTodo != null} onOpenChange={(value) => { if (!value) setDetailTodo(null) }} updateTodo={updateTodo} addSubtask={addSubtask} initialMode="view" />
