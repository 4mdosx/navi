...
export async function syncToday(date = key()) {
  ...
  const todos = await db.selectFrom('todos').selectAll()
    .where('placement', '=', 'week_plan').where('status', 'not in', ['done', 'cancelled']).execute()
  const now = new Date().toISOString()
  const sunday = new Date(`${date}T00:00:00`)
  sunday.setDate(sunday.getDate() - sunday.getDay())
  const weekStart = key(sunday)
  for (const todo of todos.filter((item) => item.weekStart === weekStart && item.dayIndex === new Date(`${date}T00:00:00`).getDay())) {
