...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (body.operation) {
      ...
      case 'todo.import_outline': data = await importOutline(args as any); break
      case 'todo.link_time': data = await linkTodoTime(String(args.id), args.grain, args.date); break
      case 'todo.unlink_time': data = await unlinkTodoTime(String(args.id), args.grain, args.date); break
      case 'todo.delete':
